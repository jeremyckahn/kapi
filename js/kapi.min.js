...
-1;this.fCount=0;for(d in this._params.styles)if(this._params.styles.hasOwnProperty(d)){this._params.styles[d]=this._params.styles[d].toString().toLowerCase();if(d==="height"||d==="width"||d==="top"||d==="left")this._params.styles[d].match(/px/)||(this._params.styles[d]+="px");a=this.el.style;b=d;c=this._params.styles[d];if(typeof c==="string")c=c.replace(/\s/g,"");a[b]=c}if(this._params.styles){this._params.styles.height&&w.call(this.el,"height");this._params.styles.width&&w.call(this.el,"width")}delete this.init;
return this},getVersion:function(){return"0.2.2"},isPlaying:function(){return this._isStopped===false&&this._isPaused===false},play:function(){var a;a=+new Date;if(this.isPlaying())return this;this._isStopped=this._isPaused=false;if(!this._startTime)this._startTime=a;if(this._loopStartTime){a-=this._pausedAtTime;this._loopStartTime+=a;this._startTime+=a}else this._loopStartTime=a;this._updateState();return this},pause:function(){clearTimeout(this._updateHandle);this._pausedAtTime=+new Date;this._isPaused=
true;return this},stop:function(){var a;clearTimeout(this._updateHandle);delete this._loopStartTime;delete this._pausedAtTime;this._isStopped=true;this._repsRemaining=-1;for(a in this._actorStateIndex)if(this._actorStateIndex.hasOwnProperty(a))this._actorStateIndex[a].queue=[];this.ctx.clearRect(0,0,this.el.width,this.el.height);return this},repeat:function(a){this._repsRemaining=typeof a==="number"&&a>=-1?parseInt(a,10)+1:-1;return this.play()},iterate:function(a){typeof a==="number"&&a>=-1&&this.repeat(a-
1);return this.play()},add:function(a,b){var c={},d=["setup","draw","teardown"],e,f=function(){return function(){}};if(a instanceof Function)c={setup:function(){},draw:a,teardown:function(){}};else if(a instanceof Object){a.draw instanceof Function||console&&console.error&&console.error("Trying to add an Object as an actor, but it does not have a draw function.");for(e=0;e<d.length;e++)c[d[e]]=a[d[e]]instanceof Function?a[d[e]]:f()}else{console&&console.error&&console.error(a+" is not a valid actor Object.");
return}c.params=b;c.constructor=a.draw;if(typeof c.id==="undefined")c.id=c.params.id||c.params.name||parseInt(Math.random().toString().substr(2),10)+ +new Date;delete c.params.name;if(typeof this._actorStateIndex[c.id]==="undefined"){this._actorStateIndex[c.id]=[];this._actorStateIndex[c.id].queue=[];this._actorStateIndex[c.id].reachedKeyframes=[]}this._actors[c.id]=c;this._liveCopies[c.id]={};this._layerIndex.push(c.id);c.params.layer=this._layerIndex.length-1;c=this._addActorMethods(c);c.setup(this);
delete c.setup;return c},getActor:function(a){return this._actors[a]},getActorList:function(){var a=[],b;for(b in this._actors)this._actors.hasOwnProperty(b)&&a.push(b);return a},removeAllKeyframes:function(){for(var a in this._actorStateIndex)if(this._actorStateIndex.hasOwnProperty(a))for(;this._actorStateIndex[a]&&this._actorStateIndex[a].length;)this._actors[a].remove(n(this._actorStateIndex[a]));return this},framerate:function(a){var b,c={},d={},e={},f,h,i,g;if(a&&typeof a==="number"&&a>0){b=
this._params.fRate;b=a/b;this._params.fRate=parseInt(a,10);k(c,this._actorStateIndex);k(d,this._originalStates);k(e,this._liveCopies);a=this._reachedKeyframes.slice(0);this.removeAllKeyframes();for(f in c)if(c.hasOwnProperty(f)){for(g=c[f].length-1;g>-1;g--)this._actors[f].keyframe(b*c[f][g],d[c[f][g]][f]);for(g=0;g<c[f].queue.length;g++)this._actorStateIndex[f].queue[g].duration*=b;this._actorStateIndex[f].reachedKeyframes=c[f].reachedKeyframes.splice(0)}for(i in e)if(e.hasOwnProperty(i)){this._liveCopies[i]=
{};for(h in e[i])e[i].hasOwnProperty(h)&&this._actors[i].liveCopy(+h*b,e[i][h].copyOf)}for(g=0;g<a.length;g++)this._reachedKeyframes[g]=a[g]*b}return this._params.fRate},gotoFrame:function(a){var b=+new Date;this.isPlaying()&&this.stop();this._currentFrame=a=this._getRealKeyframe(a)%this._lastKeyframe;this._loopStartTime=this._startTime=b-a*this._params.fRate;this._pausedAtTime=b;this._reachedKeyframes=this._keyframeIds.slice(0,this._getLatestKeyframeId(this._keyframeIds));this.ctx.clearRect(0,0,
...
false;a._loopStartTime=a._startTime+parseInt((b-a._startTime)/(a._animationDuration||1),10)*a._animationDuration;a._loopLength-=a._animationDuration||a._loopLength;a._reachedKeyframes=[];a._keyframeCache={};if(a._repsRemaining>-1){a._repsRemaining--;if(a._repsRemaining===0){a.stop();a._repsRemaining=-1;return}}}a._loopPosition=a._animationDuration?a._loopLength/a._animationDuration:0;a._currentFrame=parseInt(a._loopPosition*a._lastKeyframe,10);c=a._getLatestKeyframeId(a._keyframeIds);c=c===-1?a._lastKeyframe:
a._keyframeIds[c];if(c>(n(a._reachedKeyframes)||0))a._reachedKeyframes.push(c);c=a._reachedKeyframes.length?a._reachedKeyframes.length-1:0;if(a._reachedKeyframes[c]!==a._keyframeIds[c])a._currentFrame=a._reachedKeyframes[c]=a._keyframeIds[c];if(a._currentFrame<=a._lastKeyframe){a.autoclear!==false&&a.ctx.clearRect(0,0,a.el.width,a.el.height);typeof a.events.enterFrame==="function"&&a.events.enterFrame.call(a);a._updateActors(a._currentFrame)}a._updateState()},1E3/this._params.fRate)},_updateActors:function(a){var b,
c,d,e,f,h;for(h=0;h<this._layerIndex.length;h++){b=this._layerIndex[h];if(typeof this._actorStateIndex[b][0]!=="undefined"&&a>=this._actorStateIndex[b][0]){c=this._getActorState(b);if(c!==null){e=this._actorStateIndex[b].queue;if((f=e.length)>0){if(e[0]._internals.fromState===null)e[0]._internals.fromState=c;d=this._getQueuedActionState(e);k(c,d,true);if(f!==e.length){d=this._getLatestKeyframeId(this._actorStateIndex[b]);this._keyframes[this._keyframeIds[d]][b]=c}}c.prototype.draw.call(c,this.ctx)}}this._currentState[b]=
c}},_getActorState:function(a){var b=this._actorStateIndex[a],c=this._getLatestKeyframeId(b),d,e,f,h;if(c===-1)return null;d=this._getNextKeyframeId(b,c);e=this._keyframes[b[c]][a];f=this._keyframes[b[d]][a];if(c===d&&this._lastKeyframe>0)return null;h=n(this._actorStateIndex[a].reachedKeyframes)||0;if(!this._keyframeCache[a]){this._keyframeCache[a]={from:{},to:{}};this._actorStateIndex[a].reachedKeyframes=[]}if(c!==h)if(c>h){this._actorStateIndex[a].reachedKeyframes.push(c);this._keyframeCache[a]=
{from:this._keyframeCache[a].to,to:{}}}return this._calculateCurrentFrameProps(e,f,b[c],b[d],f.easing)},_getQueuedActionState:function(a){var b=+new Date,c=a[0],d=c._internals;if(d.startTime===null)d.startTime=b;if(!d.pauseBufferUpdated)d.pauseBufferUpdated=b;if(d.pauseBufferUpdated<this._pausedAtTime){d.pauseBuffer+=b-this._pausedAtTime;d.pauseBufferUpdated=b}if(d.toState===null){d.toState={};k(d.toState,d.fromState);k(d.toState,c.state,true)}d.currFrame=(b-(d.startTime+d.pauseBuffer))/1E3*this._params.fRate;
d.currFrame>c.duration&&a.shift();return this._calculateCurrentFrameProps(d.fromState,d.toState,0,+c.duration,c.easing||d.fromState.easing,{currentFrame:d.currFrame})},_calculateCurrentFrameProps:function(a,b,c,d,e,f){var h,i,g,j,l,o,m={};e=kapi.tween[e]?e:"linear";f=f||{};for(i in a)if(a.hasOwnProperty(i)){g=a[i];j=a.prototype.id;if(typeof this._keyframeCache[j].from[i]!=="undefined")g=this._keyframeCache[j].from[i];else if(typeof g==="function"||u(g)){if(typeof g==="function")g=g.call(a)||0;else{h=
g.match(/(\+|\-|\*|\/)\=/)[0];l=this._getPreviousKeyframeId(this._actorStateIndex[j]);l=l===-1?a.prototype.params[i]||0:this._keyframes[l][j][i];g=D[h](l,+g.replace(/\D/g,""))}this._keyframeCache[j].from[i]=g}if(B(g)){l=false;j=b[i];o=b.prototype.id;if(typeof this._keyframeCache[o].to[i]!=="undefined")j=this._keyframeCache[o].to[i];else if(typeof j==="function"||u(j)){if(typeof j==="function")j=j.call(b)||0;else{h=j.match(/(\+|\-|\*|\/)\=/)[0];j=D[h](g,+j.replace(/\D/g,""))}this._keyframeCache[o].to[i]=
